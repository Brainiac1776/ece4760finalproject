...
// custom functions to draw for pico says
void drawHeart(short x, short y, char color)
{
  // Size and offset settings
  short radius = 10;
  short width = radius * 2;
  short height = radius * 1.5;

  // Draw the two top circles
  fillCircle(x, y, radius, color);
  fillCircle(x + width, y, radius, color);

  // Draw the bottom triangle or a rounded rectangle
  fillRoundRect(x - radius, y, width * 2, height, radius / 2, color);
}

void drawLives(int lives, short startX, short startY)
{
  for (int i = 0; i < lives; i++)
  {
    drawHeart(startX + i * 30, startY, RED);
  }
}